...
export const Row3 = ({ label, vals, fmtFn = fmt, winIdx, highlight = false, section = false, flipColor = false, tooltip }) => {
  if (section) return (
    <div style={{ padding: "5px 12px", background: "rgba(251,191,36,0.03)", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
      <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 8, fontWeight: 700, letterSpacing: 1.5, color: "#fbbf24", fontFamily: "var(--mono)" }}>
        {label}
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr 1fr", gap: 0, padding: "6px 12px",
      ...
      <div style={{ fontSize: 10, color: "rgba(255,255,255,0.45)", fontFamily: "var(--body)", display: "flex", alignItems: "center", gap: 4 }}>
        {label}
        <InfoTip text={tooltip} />
      </div>
      {vals.map((v, i) => {
